...
pair<int, vector<int>> dijkstra(unordered_map<int, unordered_map<int, int>> &graph, int start, int destination)
{
    int V = graph.size();

    vector<int> dist(V, INT_MAX);
    vector<int> pred(V, -1); // To store predecessor of each node
    ...
    while (!pq.empty())
    {
        ...
        for (auto it : graph[node])
        {
            ...
            {
                dist[v] = dis + w;
                pred[v] = node; // Update predecessor
                pq.push({dist[v], v});
            }
        }
    }
...
int main()
{
    ...
    pair<int, vector<int>> result = dijkstra(graph, cityMap[from], cityMap[to]);
    ...
    {
        cout << "The shortest distance from " << from << " to " << to << " is: " << shortestDistance << " units." << endl;
        cout << "The shortest path is: ";
        for (int i = 0; i < shortestPath.size(); ++i)
        {
            cout << reverseCityMap[shortestPath[i]];
